...
#' @param constraint_by_subunit Optional. If TRUE, then will constraint by subunit and constraints_subunits parameter is required. Default is FALSE
...
build_report_run<-function(outputs_base_name,
  ...
  if(length(number_scenarios_created) == 1){
    ...
    if(constraint_by_subunit==TRUE){
      area_constraint_subunit <- constraints_subunits[[1]]
    }else{
      area_constraint_subunit <- area
    }
    ...
    if(length(prj_results$Subunit)>0 & is.null(master_subunit_field) & spatial_optimization==TRUE){
      ...
      for(x in 1:all_effects){
        my_effect_chosen <- effect_fields[x]
        df_loop_effects <- prj_results[,c("ProjectNumber",paste0("ETrt_",my_effect_chosen),"Subunit",paste0("Treat_",area_constraint_subunit))]
        df_loop_effects$effect_name <- paste0("ETrt_",my_effect_chosen)
      ...
      for(x in 1:all_effects){
        my_effect_chosen <- effect_fields[x]
        df_loop_effects <- prj_results[,c("ProjectNumber",paste0("ETrt_",my_effect_chosen),paste0("Treat_",area_constraint_subunit))]
    ...
    if(length(prj_results$Subunit)>0 & !is.null(master_subunit_field) & spatial_optimization==TRUE){
      ...
      for(x in 1:all_effects){
        my_effect_chosen <- effect_fields[x]
        df_loop_effects <- prj_results_vs_master[,c("ProjectNumber",paste0("ETrt_",my_effect_chosen),"Subunit",paste0("Treat_",area_constraint_subunit))]
        df_loop_effects$effect_name <- paste0("ETrt_",my_effect_chosen)
      ...
      for(x in 1:all_effects){
        my_effect_chosen <- effect_fields[x]
        df_loop_effects <- prj_results_vs_master[,c("ProjectNumber",paste0("ETrt_",my_effect_chosen),paste0("Treat_",area_constraint_subunit))]
