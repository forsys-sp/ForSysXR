...
#' @param objectives Vector containing the field(s) from input_shapefile identifying the treatment priorities, the treatment type, minimum weight, maximum weight and step. Each objective is characterized by five elements in the vector, following the order c("objective","treatment type","min weight","max weight", "step").
  ...
  if(build_interac_report==TRUE){
    ...
    if(any(grepl("shapefile", save_outputs, fixed = TRUE))==TRUE){
      if(length(number_scenarios_created) == 1){
        ...
        palFunc_objective_1 <- leaflet::colorNumeric("RdBu",
                                                     output_shp_run[,paste("ProjectNum")][[1]])

        cols_obj1=colourvalues::colour_values_rgb(output_shp_run[,paste("ProjectNum")][[1]],  palette = "rdbu",include_alpha = FALSE)


        #palFunc_objective_1 <- leaflet::colorNumeric(viridis::turbo(nrow(output_shp_run)),
        #                                             output_shp_run[,paste("ProjectNum")][[1]])

        output_shp_run_wgs84 <-st_transform(output_shp_run,crs = 4326)
        my_shp_wgs84 <- st_transform(output_shp_run,crs = 4326)
################################
